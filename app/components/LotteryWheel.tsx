...
//     const winnerIndex =
//       Math.floor(relativeAngle / anglePerSegment) % segments.length;

//     console.log("🎯 Winner Calculation:", {
//       pointerAngle: ((pointerAngle * 180) / Math.PI).toFixed(2) + "°",
//       normalizedRotation:
//         ((normalizedRotation * 180) / Math.PI).toFixed(2) + "°",
...
//     const spinDuration = 15000; // 15 seconds
//     const minSpins = 6;
//     const maxSpins = 10;
...
//           🎄
//         </div>
//         <div className="absolute -top-4 -right-4 text-4xl animate-bounce delay-300">
//           ⭐
//         </div>
//         <div className="absolute -bottom-4 -left-4 text-4xl animate-bounce delay-500">
//           🎁
//         </div>
//         <div className="absolute -bottom-4 -right-4 text-4xl animate-bounce delay-700">
//           ❄️
//         </div>
...
//             0 0 80px rgba(249, 115, 22, 0.4), 0 10px 30px rgba(0, 0, 0, 0.3);
//         }
...
export default function LotteryWheel({
  ...
  const ft0371WonRef = useRef(false);
  ...
  const spinWheel = () => {
    ...
    if (currentSpin === 4 && !ft0371WonRef.current) {
      const targetIndex = segments.findIndex(s => 
        (s.ftCode === 'FT0371') || (s.ft_code === 'FT0371')
      );
      ...
      if (targetIndex !== -1) {
        ...
        const anglePerSegment = (2 * Math.PI) / segments.length;
        const pointerAngle = ((3 * Math.PI) / 2) % (2 * Math.PI);
        const targetSegmentStartAngle = targetIndex * anglePerSegment;
        const targetSegmentMiddle = targetSegmentStartAngle + (anglePerSegment / 2);
        const baseRotations = 8;
        const baseRotation = baseRotations * 2 * Math.PI;
        
        let rotationToAlign = pointerAngle - targetSegmentMiddle;
        
        while (rotationToAlign < 0) {
          rotationToAlign += 2 * Math.PI;
        }
        while (rotationToAlign >= 2 * Math.PI) {
          rotationToAlign -= 2 * Math.PI;
        }
        
        const finalRotation = baseRotation + rotationToAlign;
    ...
    const animate = () => {
      ...
      if (progress < 1) {
        ...
        if ((segments[actualWinner]?.ftCode === 'FT0371') || 
            (segments[actualWinner]?.ft_code === 'FT0371')) {
          ft0371WonRef.current = true;
        }
